...
- [x] **`foreach` statement** — `for ( [specifiers] name : type = source_expr ) nested_stmt`, syntactically identical for all three klangc source kinds (array, `::k::Iterator`/`ConstIterator`, `Sequence`/`MutableSequence` — resolved later by type checking, not by the grammar). New `foreachStatement`/`foreachVarDecl` rules, tried before `forStatement` (a trailing `;` after the init expression rolls back to the classic `for`). `foreachVarDecl` is a `KlangNamedElement` (rename/navigation) and its variable is visible in the loop body via `KlangResolveUtil.collectFromScope`. Tests: `KlangForeachStatementParsingTest`.
- [x] **Primitive array-literal temporaries** — a bare simple primitive-type keyword (`bool`/`byte`/`char`/`short`/`int`/`long`/`float`/`double`, not `unsigned`/compound forms) immediately followed by `[` is now a valid `primaryExpr` (`primitiveArrayElementType`), enabling `int[]{1, 2, 3}` as a foreach/expression source, mirroring the compiler's `parser.hpp::is_simple_primitive_type_keyword()`.
- [x] **`final` forbidden on constructors/destructors** — new `KlangFinalOnCtorDtorInspection` flags `final` on a ctor/dtor (klangc `ERR_FINAL_ON_CTOR_DTOR`, 0x0195; neither can be inherited/overridden). Tests: `KlangFinalOnCtorDtorInspectionTest`.
